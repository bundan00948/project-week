// Import required Firebase modules
    ...
    onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        if (isAuthRequiredPage(getCurrentPageId())) {
          window.location.assign('/games/dashboard');
        }
      }
    });
    ...
    function isAuthRequiredPage(pageId) {
      return pageId !== 'main-page' && pageId !== 'movies-page' && pageId !== 'home' && pageId !== 'contact';
    }
    ...
    async function loadActivePageContent(pageId) {
      switch (pageId) {
        ...
        case 'home':
        case 'contact':
          if (getRequestedGameIdFromUrl()) await mountMainPageGamesContent();
    ...
    document.querySelectorAll('.sidebar-tabs .tab-button, #home-tab, #contact-tab').forEach((link) => {
      link.addEventListener('click', (e) => {
        const pageId = link.getAttribute('data-page') || (link.id === 'home-tab' ? 'home' : link.id === 'contact-tab' ? 'contact' : '');
        if (!pageId || !isAuthRequiredPage(pageId)) return;
        if (!currentUser) {
          e.preventDefault();
          if (!authStateKnown) {
            pendingPageRoute = pageId;
            return;
          }
          noticeModal.style.display = 'flex';
        }
